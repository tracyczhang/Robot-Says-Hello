...
def listen_print_loop(responses, stream, file):
    ...
    transcribe_streaming_mic.listen_print_loop(
            _record_keeper(with_results, stream), file)
...
def main(sample_rate, audio_src):
    ...
    #xmlName = fileId+ ".xml"
    txtName = fileId + ".txt"
    #prettyXmlName = fileId + "pretty.xml"
    file = open("test.txt", "w")
    file.write("Start,Text,End\n")
    file.write(fileId + ",")
    ...
    with mic_manager as stream:
        ...
        while True:
            ...
            try:
                ...
                listen_print_loop(responses, stream, file)
                break
